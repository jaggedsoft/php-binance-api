...
class API
{
    ...
    protected $fapiTestnet = 'https://testnet.binancefuture.com/'; // /< Testnet REST endpoint for the futures API
    ...
    /**
     * continuousKlines get the candles for the given intervals
     * 1m,3m,5m,15m,30m,1h,2h,4h,6h,8h,12h,1d,3d,1w,1M
     *
     * $candles = $api->continuousKlines("BTCUSDT", "5m", 500, 1607444700000, 1607444759999);
     *
     * @param $pair string to query
     ...
     * @throws \Exception
     */
    public function continuousKlines(string $pair,string $contractType, string $interval = "5m", int $limit = 500, $startTime = null, $endTime = null)
    {
        if (!isset($this->charts[$pair])) {
            $this->charts[$pair] = [];
        }
        ...
        $response = $this->httpRequest("fapi/v1/continuousKlines", "GET", $opt, false);
